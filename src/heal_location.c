...
void SetWhiteoutRespawnWarpAndHealerNpc(struct WarpData * warp)
{
    ...
    else
    {
        ...
        if (sWhiteoutRespawnHealCenterMapIdxs[healLocationIdx - 1][0] == MAP_GROUP(MAP_RECRUIT_BASE_ENTRANCE) && sWhiteoutRespawnHealCenterMapIdxs[healLocationIdx - 1][1] == MAP_NUM(MAP_RECRUIT_BASE_ENTRANCE))
        {
            warp->x = 8;
            warp->y = 5;
        }
