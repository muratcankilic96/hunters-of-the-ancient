...
const u8 *const gStdStringPtrs[] = {
    ...
    [STDSTRING_BOULDER_ARTIFACT]    = gText_Boulderartifact,
    [STDSTRING_CASCADE_ARTIFACT]    = gText_Cascadeartifact,
    [STDSTRING_THUNDER_ARTIFACT]    = gText_Thunderartifact,
    [STDSTRING_RAINBOW_ARTIFACT]    = gText_Rainbowartifact,
    [STDSTRING_SOUL_ARTIFACT]       = gText_Soulartifact,
    [STDSTRING_MARSH_ARTIFACT]      = gText_Marshartifact,
    [STDSTRING_VOLCANO_ARTIFACT]    = gText_Volcanoartifact,
    [STDSTRING_EARTH_ARTIFACT]      = gText_Earthartifact,
...
static void CreatePCMenuWindow(void)
{
    ...
    if (FlagGet(FLAG_SYS_GAME_CLEAR))
    {
        ...
        SetStdWindowBorderStyle(windowId, FALSE);
        AddTextPrinterParameterized(windowId, FONT_NORMAL, gText_BossPc, cursorWidth, 34, TEXT_SKIP_DRAW, NULL);
