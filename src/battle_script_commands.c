...
static void Cmd_accuracycheck(void)
{
    ...
    {
        u8 damageCategory, moveAcc, holdEffect, param;
        s8 buff;
        u16 calc;
        ...
        GET_MOVE_DAMAGE_CATEGORY(move, damageCategory);
        ...
        if (gBattleMons[gBattlerAttacker].ability == ABILITY_HUSTLE && IS_TYPE_PHYSICAL(damageCategory))
...
static void Cmd_datahpupdate(void)
{
    ...
    damageCategory = gBattleMoves[gCurrentMove].damageCategory;
    ...
    if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT))
    {
        ...
        else
        {
            ...
            else // hp goes down
            {
                ...
                if (IS_TYPE_PHYSICAL(damageCategory) && !(gHitMarker & HITMARKER_PASSIVE_DAMAGE) && gCurrentMove != MOVE_PAIN_SPLIT)
                {
                ...
                else if (!IS_TYPE_PHYSICAL(damageCategory) && !(gHitMarker & HITMARKER_PASSIVE_DAMAGE))
                {
