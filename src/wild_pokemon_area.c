...
static bool32 IsSpeciesOnMap(const struct WildPokemonHeader * data, s32 species)
{
    if (IsSpeciesInEncounterTable(data->landMonsDayInfo, species, LAND_WILD_COUNT))
        return TRUE;
    if (IsSpeciesInEncounterTable(data->waterMonsDayInfo, species, WATER_WILD_COUNT))
        return TRUE;
    if (IsSpeciesInEncounterTable(data->fishingMonsDayInfo, species, FISH_WILD_COUNT))
        return TRUE;
    if (IsSpeciesInEncounterTable(data->rockSmashMonsDayInfo, species, ROCK_WILD_COUNT))
        return TRUE;
